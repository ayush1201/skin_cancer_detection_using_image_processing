...
for j=1:nb_class-1
   mat=M(j,:);
    if (mat(1)<=nb_class && mat(2)<=nb_class)
        ...
        svmstruct{n}=fitcsvm([Classes{n,1};Classes{n,2}],[train_label{mat(1)};train_label{mat(2)}],'Showplot',false, 'Kernel_Function','rbf');
        n=n+1;
    elseif (mat(1)> nb_class && mat(2)> nb_class)
       ...
       svmstruct{n}=fitcsvm([Classes{n,1};Classes{n,2}],[-nb_class*ones(f1,1);-(nb_class+1)*ones(k1,1)],'Showplot',false, 'Kernel_Function','rbf');
       ...
       A2=[];
        ...
        Classes{n,1}=train_data{mat(mat<=nb_class)};
         ...
         svmstruct{n}=fitcsvm([Classes{n,1};Classes{n,2}],[train_label{mat(mat<=nb_class)};-1*cluster_ind*ones(l1,1)],'Showplot',false, 'Kernel_Function','rbf');
         n=n+1;
         C1=[];
         C2=[];
    end
